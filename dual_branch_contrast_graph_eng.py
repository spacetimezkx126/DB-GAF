...
def load_data_coliee(datas, train_text):
    case_dataset = CaseDataset(datas, dataset = 'coliee')
    train_indices = []
    val_indices = []
...
class CaseDataset(Dataset):
    def __init__(self, data, dataset = 'coliee', part_few = 'None'):
        ...
        texts = []
        with open("./dataset/coliee_contrast_graph.pkl",'rb')as f:
    ...
    def cache_all_data(self):
        ...
        save_to_pkl(self.cached_data, 'coliee_contrast_graph.pkl')
    ...
    def build_graph(self, case):
        ...
        branch1_node = f"branch1_attributes_{case['Case ID']}"
        branch2_node = f"branch2_attributes_{case['Case ID']}"
        G.add_node(branch1_node, **default_node_attributes, node_type="virtual", case_number=case['Case ID'], name="important")
        G.nodes[branch1_node]["texts"] = "important"
        G.add_edge(case_id, branch1_node, edge_type="has_virtual",crime_name="None")
        G.add_node(branch2_node, **default_node_attributes, node_type="virtual", case_number=case['Case ID'], name="unimportant")
        ...
        for crime_info in case["Crimes"]:
            ...
            if crime_name1 in origin_text:
                ...
                G.nodes[crime_node1]["texts"] = crime_name+":"+crime_name1+":"+origin_text[crime_name1]
            
            crime_node_branch1 = f"crime_important_{crime_name}_{case['Case ID']}"
            G.add_node(crime_node_branch1, **default_node_attributes, node_type="crime",case_number=case['Case ID'],name=crime_name)
            ...
            crime_node_branch2 = f"crime_unimportant_{crime_name}_{case['Case ID']}"
            G.add_node(crime_node_branch2, **default_node_attributes, node_type="crime",case_number=case['Case ID'],name=crime_name)
...
if __name__ == '__main__':
    ...
    label_dic = json.load(open("./labels/coliee.json","r"))
    train_test_v2 = {"train": train_test["coliee"]["train"],"test": train_test["coliee"]["test"]} 
    # part_few = args.attribute_file
    train_loader, val_loader = load_data_coliee(datas, train_test_v2)
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                if len(sim_3)!=0 and len(sim_other)!=0:
                    # loss1 = margin_ranking_loss(torch.stack(sim_3),torch.stack(sim_other))
                    # +ranking_loss_with_metrics(data_json,label_dic)
