...
parser.add_argument('--checkpoint', type = str, default = './checkpoints/basic_graph_eagatv2_efwf1.pth', 
                    help = 'load the checkpoint for testing')
parser.add_argument('--test_out_file', type = str, default = './results/result_1_1.json', 
                    help = 'load the checkpoint for testing')
...
def load_data_coliee(datas, train_text, part_few = 'None'):
    case_dataset = CaseDataset(datas, dataset = 'coliee', part_few = part_few)
    train_indices = []
    val_indices = []
    for idx, case_batch in enumerate(case_dataset.cached_data):
        case_number = case_batch.case_number 
        if case_number[0] in train_text['test']:
...
class CaseDataset(Dataset):
    def __init__(self, data, dataset = 'coliee', part_few = 'None'):
        self.data_list = data
        self.device = torch.device("cuda:0")
        if dataset == 'coliee':
            origin_text_dir = "./dataset/processed_text/coliee2025/"
        with open(origin_text_dir+"fact.json","r",encoding='utf-8')as f:
        ...
        if part_few is not None:
            ...
            self.part_few = None
        with open("./basic_coliee_high_low.pkl",'rb')as f:
    ...
    def cache_all_data(self):
        ...
        save_to_pkl(self.cached_data, 'basic_coliee_high_low.pkl')
    ...
    def build_graph(self, case):
        ...
        PAD = '<PAD>'
        UNK = '<UNK>'
        case_id = f"case_{case['Case ID']}"
        origin_text = None
        # print("1127**",self.part_few)
        # print(case['Case ID'],self.data_doc.keys(), self.data_query.keys())
        # if case['Case ID'] in self.data_doc:
        #     origin_text = self.data_doc[case['Case ID']]
        if case['Case ID'] in self.data_query:
            origin_text = self.data_query[case['Case ID']]
        G.add_node(case_id, **default_node_attributes, node_type = "case",case_number = case['Case ID'], name = case_id)
        content = []
        G.nodes[case_id]["texts"] = "None"
        zuiming = []
        for crime_info in case["Crimes"]:
            crime_name = crime_info["Crime_Type"]
            if type(crime_name)==list:
                crime_name = crime_name[0]
            crime_name1 = crime_info["Crime_Name"]
            if type(crime_name1)==list:
                crime_name1 = crime_name1[0]
            # if crime_name not in zuiming:
            zuiming.append(crime_name)
            crime_node = f"crime_{crime_name}_{case['Case ID']}"
            if crime_name1 in origin_text:
                crime_node1 = crime_name1
                # if "经过：" in origin_text[crime_name]:
                G.add_node(crime_node1, **default_node_attributes, node_type = "crime1", case_number = case['Case ID'], name = crime_name+":"+crime_name1+":"+origin_text[crime_name1])
                G.add_edge(case_id, crime_node1, edge_type = "has_crime1", crime_name = crime_name, part = 0)
                G.nodes[crime_node1]["texts"] = crime_name+":" + origin_text[crime_name1]
            G.add_node(crime_node, **default_node_attributes, node_type = "crime", case_number=case['Case ID'], name = crime_name)
            ...
            for section, content in crime_info.items():
                section_node = f"{section}_{crime_name}_{case['Case ID']}"
                            ...
                            if self.part_few is not None:
                                if crime_name in self.part_few:
                                    if f"{section}" in self.part_few[crime_name]:
                                        G.add_node(section_node, **default_node_attributes, node_type="attribute", case_number = case['Case ID'], name = cont)
                                        drop = 0
                                        if crime_name in self.part_few:
                                            if f"{section}" in self.part_few[crime_name]:
                                                drop = 1
...
class Basic_Graph_Model(nn.Module):
    ...
    def forward(self, data, drop_mode = False, drop_type = 'few', mode = 'train'):
        ...
        if self.model_type != 'CaseGNN':
            ...
            if mode == 'test':
                f_m = calculate_macs_flops(self.graph_model_l1, data)
                it = measure_inference_time(self.graph_model_l1, data)
                memo = measure_memory_footprint(self.graph_model_l1, data)
            
            x2, (adj, alpha1) = self.graph_model_l2(x1, filtered_edge_index, edge_attr = filtered_edge_attr, return_attention_weights=True)
            x2 = F.relu(x1 + x2)
            if mode == 'test':
                return x2, f_m, it, memo
            else:
                return x2
...
def calculate_macs_flops(model, data):
    """计算MACs和FLOPs"""
    model.eval()
...
def measure_memory_footprint(model, data):
    """测量内存占用"""
    model.eval()
    if isinstance(data, tuple):
        x, edge_index = data
...
if __name__ == '__main__':
    label_json = "./labels/coliee.json"
    dataset_dir = "./dataset/coliee"    
    ...
    train_test_v2 = {"train": train_test["coliee"]["train"],"test": train_test["coliee"]["test"]} 
    part_few = args.attribute_file
    train_loader, val_loader = load_data_coliee(datas, train_test_v2, part_few = part_few)
    # print("2068",mode, times, epochs)
    if mode != 'test':
        for n in range(times):
            ...
            for epoch in range(epochs):
                ...
                for batch in train_loader:
                    ...
                    data = batch[0]
                    # f_m = calculate_macs_flops(model, data)
                    # it = measure_inference_time(model, data)
                    # memo = measure_memory_footprint(model, data)
                    # print(f_m, it, memo)
                    out = model(data)
                    # print(data,data.case_number)
                    ...
                    sim_2_mask = unique_case_labels == 3
                ...
                with torch.no_grad():
                    for batch in val_loader:
                        ...
                        case_numbers1 = list(filter(lambda x: x != case_query, case_numbers))
                        unique_cases = list(set(case_numbers1)) 
                        case_all = case_all + len(unique_cases) + 1
                        first_indices =[]
                        for i in range(len(unique_cases)):
                            for j in range(len(case_numbers)):
                                if case_numbers[j] == unique_cases[i] and data.node_type[j] =="case":
                    ...
                    # 'macs': 186980812.0, 'flops': 1443895756, 'params'
                    print(count_macs, count_flops, count_it, count_memo,case_all, count_macs/case_all,count_flops/case_all, count_it/case_all, count_memo/case_all)
                    # break
                # break
            # break
    else:
